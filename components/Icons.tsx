...
import {
  ...
  List, Palette, Type, Settings, Key, ExternalLink
} from 'lucide-react';
...
export const IconSettings: React.FC = React.memo(() => <Settings className="w-4 h-4" />);
export const IconKey: React.FC<{ className?: string }> = React.memo(({ className }) => <Key className={className || "w-5 h-5 stroke-current"} />);
export const IconExternalLink: React.FC = React.memo(() => <ExternalLink className="w-3 h-3 inline ml-1" />);
